...
export default {
  ...
  methods: {
    ...
    async fetchTableStructure() {
      try {
        const response = await fetch(`/api/databases/${this.databaseName}/tables/${this.tableName}`)
    ...
    async fetchTableList(query) {
      ...
      try {
        const response = await fetch(`/api/databases/${this.databaseName}/tables`)
    ...
    async confirmImport() {
      ...
      try {
        ...
        // 调用API进行导入
        const response = await fetch(`/api/databases/${this.databaseName}/import/csv`, {
          method: 'POST',
