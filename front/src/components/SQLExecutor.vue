...
export default {
  ...
  methods: {
    ...
    async fetchTables() {
      ...
      try {
        const response = await fetch(`/api/databases/${this.connectionInfo.name}/tables`)
    ...
    async insertTableName(tableName) {
      ...
      try {
        ...
        const response = await fetch(`/api/databases/${this.connectionInfo.name}/tables/${tableName}`);
    ...
    async confirmExport() {
      ...
      try {
        ...
        // 调用后端导出接口
        const response = await fetch(`/api/databases/${this.connectionInfo.name}/export`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
